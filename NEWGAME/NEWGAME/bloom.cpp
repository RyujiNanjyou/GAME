...
{
	//ブルームのシェーダーをロード
	effect = effectmanager->LoadEffect("Assets/bloom.fx");
	...
	downSamplingRenderTarget[0].Create(
		WINDOW_WIDTH >> 1,	//横の解像度をフレームバッファの半分にする。
		WINDOW_HEIGHT,
		1,
	...
	downSamplingRenderTarget[1].Create(
		WINDOW_WIDTH >> 1,	//縦と横の解像度をフレームバッファの半分にする。
		WINDOW_HEIGHT >> 1,
		1,
		D3DFMT_A16B16G16R16F,	//ここも浮動小数点バッファにする。
		D3DFMT_D16,				//使わないので16bit。本来は作成する必要もない。
...
void Bloom::Render()
{
	...
	//まずは輝度を抽出する。
	{
		...
		effect->BeginPass(0);
		//シーンテクスチャを設定する。
		effect->SetTexture("g_scene", rendertarget->GetTexture());
		//定数レジスタへの変更をコミット。
